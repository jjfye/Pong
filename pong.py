...
#capitalized naming convention indicates CONSTANT variables
WIDTH, HEIGHT = 700, 500
WINDOW = pygame.display.set_mode((WIDTH, HEIGHT)) #sets the size of the window
pygame.display.set_caption("Pong") #names the application
...
class Paddle:
    ...
    def move(self, up=True): #allows up-arrow key to move paddle up/down
        if up:
...
def draw(window, paddles, ball, left_score, right_score):
    ...
    for i in range(10, HEIGHT, HEIGHT//20):
        if i % 2 == 1:
            continue
        pygame.draw.rect(window, WHITE, (WIDTH//2 - 2.5, i, 5, HEIGHT//20))
...
def main():
    ...
    #initiate event loop until quit is clicked.
    while run:
        clock.tick(FPS)
